...
$url = "directing_data_write/json/directing_data_2.json";
...
$fp = fopen("csv/directing_data_2.csv",'wb');
flock($fp, LOCK_EX);
ftruncate($fp,0);
flock($fp, LOCK_UN);
fclose($fp);
...
for($i=0;$i<$data_size;$i++){
    for($j=0;$j<6;$j++){
        ...
        file_open_write($decoded_json2[$i][$data_name[$j]],"csv/directing_data_2.csv");
    }
}
}