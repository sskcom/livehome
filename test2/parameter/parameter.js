    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (number) {
            ...
            for (var j = 0; j < number - 1; j++) {
                ...
                var id_element_width = Number($(id).width());


                // var time = Math.round(id_element_width * 0.2399130853932732);
                console.log("width:"+id_element_width);
                // var time = Math.round(id_element_width / par_data_size);
                var time = Math.round(id_element_width/72);
                console.log("t3:"+time)
...
$(document).on("mouseover", ".i-i", function (e) {
    ...
    // $("#" + id).css("resize", "horizontal");
    $("#"+id).resizable({
        minHeight:edit_height,
        maxHeight:edit_height,
        minWidth:72,
        maxWidth:21600,
        String:"enable"	,
        scroll: true,
...
function buttonAdd() {
    ...
    // $("#i_" + e_number + "_" + number).css("content","mode 1");
...
function all_element_ajax(event, selector, trigger) {
    $(document).on(event, selector, function () {
        ...
        var now_id_element_width = Number($(now_id).width());
        
        
        // var time = Math.round(now_id_element_width * 0.2399130853932732);
        
        // var time = Math.round(now_id_element_width/par_data_size);
        var time = Math.round(now_id_element_width/72);
       
        if (time >= 60) {
            var time_1 = Math.floor(time / 60);
            var time_2 = time % 60;
        ...
        $('#input' + text_number).prop('checked', true);
...
$(document).on("click", "#btn3", function () {
    ...
    Ajax("2," + folder_number, "http://localhost/test/play_trigger_write.php");//ローカル用
...
$(document).on("click", "#btn4", function () {
    ...
    location.href = "http://localhost/test/test2.php";//ローカル用
});
...
function $click_Ajax(event, selector, trigger) {
    ...
    $(document).on(event, selector, function (e) {
        ...
        $(edit_element).css('width', (time_value *72) + "px");
        ...
        if (trigger == 1) {
            ...
            Ajax("0," + folder_number, "http://localhost/test/play_trigger_write.php");//ローカル用

            switch (now_edit_id) {
                case 1:
                    // Ajax(1, "http://54.248.37.203/test/play_trigger_write_2.php");
                    Ajax(1, "http://localhost/test/play_trigger_write_2.php");//ローカル用
                    break;
                case 2:
                    // Ajax(2, "http://54.248.37.203/test/play_trigger_write_2.php");
                    Ajax(2, "http://localhost/test/play_trigger_write_2.php");//ローカル用
                    break;
                case 3:
                    // Ajax(3, "http://54.248.37.203/test/play_trigger_write_2.php");
                    Ajax(3, "http://localhost/test/play_trigger_write_2.php");//ローカル用
                    break;
                case 4:
                    // Ajax(4, "http://54.248.37.203/test/play_trigger_write_2.php");
                    Ajax(4, "http://localhost/test/play_trigger_write_2.php");//ローカル用
                    break;
